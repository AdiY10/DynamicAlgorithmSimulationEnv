...
class DynamicSampling:
    def __init__(self):
        ...
        print('-----', percentage_of_bugs_remaining_1)
        ...
        print("--- ",percentage_of_bugs_remaining_2)
        ...
        while self.goal_1.target_pose.pose.position.x < 6.5:
            ...
            if(self.x_pos<self.num_of_rows):
                if (self.bugs[self.x_pos,self.y_pos]):
                    ...
                    while(self.bugs[self.x_pos,self.y_pos]):
            ...
            self.goal_1.target_pose.pose.position.x+=1.0
        if(not self.first_goal):
            self.goal_1.target_pose.pose.position.y=0.55#0.53
        ...
        while self.goal_3.target_pose.pose.position.x >=0.0:
            ...
            if(self.x_pos<self.num_of_rows):
                if (self.bugs[self.x_pos,self.y_pos]):
                    ...
                    rospy.logwarn("bug not found")
            self.goal_3.target_pose.pose.position.x-=1.0
        return value
    ...
    def day1(self):
        ...
        for x in range(int(1)):
            if self.goal1():
                self.goal_1.target_pose.pose.position.y+=2*1.4
                if self.sent_next_goal(self.goal_2):
                    self.goal_2.target_pose.pose.position.y+=2*1.4
                    if self.goal3():
                        self.goal_3.target_pose.pose.position.y+=2*1.4
                        if self.sent_next_goal(self.goal_4):
                            self.goal_4.target_pose.pose.position.y+=2*1.4
